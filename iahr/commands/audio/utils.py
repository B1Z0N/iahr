...
class FileAudioSegment:
    def __init__(self, path):
        self.set_path(path)
        self.track = AudioSegment.from_file(self.path,
                                            format=self.pydub_extension)
    ...
    def set_path(self, path):
        ...
        self.pydub_extension = self.TELEGRAM_TO_PYDUB_FORMATS.get(
            self.tg_extension, self.tg_extension)
...
class AudioSender(ABCSender):
    async def send(self):
        ...
        await self.event.client.send_file(cid,
                                          outfile,
    ...
    async def invoke(self, *args, **kwargs):
        if len(args) >= 1 and isinstance(args[0], FileAudioSegment):
            ...
            self.res = audiof.move(await self.fun(*args, **kwargs))
              ...
              self.event.message.get_reply_message()) is not None:
            ...
            IahrDocumentSizeTooLarge.check(reply.document.size,
                                           'audio_file_max_size_mb', 15)

            audiof = FileAudioSegment(
                await reply.download_media(file=IahrConfig.MEDIA_FOLDER))
