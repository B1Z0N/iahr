...
TRUE_PREFIXES = { 
    getattr(events, evstr): pref for evstr, pref in PREFIXES.items()
}
...
def test_config():
    ...
    cfg(left='{', right='}', raw='e', cmd='!', prefixes=PREFIXES,
        me='my', others='/', log_format='%(name)s', log_datetime_format='%m',
        local='russian', data_folder='folder', custom={ 
            'current_entity' : '_' 
        }, mode='DEBUG')
    ...
    assert icfg.REVERSE_PREFIXES == dict(zip(TRUE_PREFIXES.values(), TRUE_PREFIXES.keys()))
    assert icfg.ME == 'my'
    assert icfg.OTHERS == '/'
